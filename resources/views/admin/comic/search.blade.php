                ...
                <div class="main-content">
                    <div class="row">
                        ...
                        <div class="col-lg-12" id="list_comic_admin">
                            <?php $k=0; foreach($all_comic as $key => $comic){ $k++; ?>
                            <div class="col-lg-2">
                                <div class="card alert">
                    ...
                    <div class="col-lg-12 phantrang_vip" style="text-align:center;">
                        <?php $sotruyentrongtrang = 6;
                           $tongsotruyen = 0;
                           $tranghientai  = $tranghientai_vip;
                           foreach($tongsotruyen_vip as $key => $value){$tongsotruyen++;}
                           $sotrang = ceil($tongsotruyen / $sotruyentrongtrang);
                           if ($tranghientai > $sotrang){
                            ...
                            $batdau = ($tranghientai - 1) * $sotruyentrongtrang;
                            if ($tranghientai > 1 && $sotrang > 1){
                                echo '<a class="phantrang_element" href="/phantrang/'.$cate_phantrang.'/'.$comic_phantrang.'/'.($tranghientai-1).'"><</a>';
                            }
                            for ($i = 1; $i < $sotrang; $i++){
                               
                                if ($i == $tranghientai){
                                    echo '<span class="phantrang_element">'.$i.'</span>';
                                }
                                else{
                                    echo '<a class="phantrang_element" href="/phantrang/'.$cate_phantrang.'/'.$comic_phantrang.'/'.$i.'">'.$i.'</a>';
                                }
                            }
                            
                            if ($tranghientai < $sotrang - 1 && $sotrang > 1){
                                echo '<a class="phantrang_element" href="/phantrang/'.$cate_phantrang.'/'.$comic_phantrang.'/'.($tranghientai+1).'">></a>';
                            }
                        ?>
                    </div>
